class RestApi {
    ...
    async getEquipment(equipment) {
        ...
        // I added "like" condition so you can filter your keyup text

        const playersResponse = await fetch(
            `http://localhost:3000/players?name_like=${equipment}`
        );
        const players = await playersResponse.json();
        console.log("players:", players);

        const mixersResponse = await fetch(
            `http://localhost:3000/mixers?name_like=${equipment}`
        );
        const mixers = await mixersResponse.json();
        console.log("mixers:", mixers);

        const controllersResponse = await fetch(
            `http://localhost:3000/controllers?name_like=${equipment}`
        );
        const controllers = await controllersResponse.json();
        console.log("controllers:", controllers);

        const turntablesResponse = await fetch(
            `http://localhost:3000/turntables?name_like=${equipment}`
        );
